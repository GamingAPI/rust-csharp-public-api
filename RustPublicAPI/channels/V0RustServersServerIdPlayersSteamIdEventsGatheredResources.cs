...
String server_id,String steam_id
  )
  {
    ...
    {
      logger.Debug("Got message for channel subscription: " + $"v0.rust.servers.{server_id}.players.{steam_id}.events.gatheredResources");
      var deserializedMessage = JsonDeserializerSupport(logger, (byte[])args.ReceivedObject);

      var unmodifiedChannel = "v0.rust.servers.{server_id}.players.{steam_id}.events.gatheredResources";
...
steamIdParam);
    ...
    logger.Debug("Subscribing to: " + $"v0.rust.servers.{server_id}.players.{steam_id}.events.gatheredResources");
    return connection.SubscribeAsync($"v0.rust.servers.{server_id}.players.{steam_id}.events.gatheredResources",handler);
  }
